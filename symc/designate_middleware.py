...
class DesignateMiddleware(object):
    ...
    def _find_designate_domain(self, dom_name, env):
        dom_name = dom_name + '.'
        url = '%s/domains' % (self._conf['designate_url'])
        resp = requests.get(url, headers={'X-Auth-Token':
    ...
    def _create_designate_domain(self, dom_name, X_Auth_Token, email,
                                 ttl=3600):
        conf = self._conf
        ...
        ttl = ttl
        email = email.lower()
        self._logger.debug('Designate domain %s not found, creating..',
                           dom_name)
        body = {'name': dom_name,
                'ttl': int(ttl),
                'email': email}
        ...
        if resp.status_code == 200:
                ...
                self._logger.info('Designate domain %s created(id: %s).',
                                  dom_name, dom_id)
                return dom_id
        else:
                self._logger.error('Error in creating domain code %s msg %s',
    ...
    def _delete_designate_domain(self, dom_name, dom_id, X_Auth_Token):
        ...
        resp = requests.delete(url,
                               headers={'X-Auth-Token': token,
                                        'Content-Type': 'application/json'})
        if resp.status_code == 200:
            self._logger.info('Designate domain(%s) with id %s deleted',
                              dom_name, dom_id)
        else:
            (self._logger.error("Error designate domain %s delete, response"
                                "status %s text %s" %
                                (dom_name, resp.status_code, resp.text)))
            return
    # END of _delete_designate_domain

    # Start _create_designate_record
    def _create_designate_record(self, dom_id, rec_name, X_Auth_Token,
                                 rec_type, rec_ip, ttl=3600):
        conf = self._conf
        dom_id = dom_id
        rec_name = rec_name + '.'
        ...
        resp = requests.post(url, data=json.dumps(body),
                             headers={'X-Auth-Token': token,
                                      'Content-Type': 'application/json'})
        if resp.status_code == 200:
            (self._logger.info('Designate "%s" record "%s" created(addr: %s)'
                               % (rec_type, rec_name, fip_addr)))
        else:
            (self._logger.error("Error creating designate record(%s),"
                                "response status %s text %s"
                                % (rec_name, resp.status_code, resp.text)))
            return
    ...
    def _delete_designate_record(self, dom_id, rec_name, rec_id, X_Auth_Token):
        ...
        rec_name = rec_name
        token = X_Auth_Token
        url = (conf['designate_url'] + '/domains/%s/records/%s' %
               (dom_id, rec_id))
        resp = requests.delete(url,
                               headers={'X-Auth-Token': token,
                                        'Content-Type': 'application/json'})
        if resp.status_code == 200:
            ...
            (self._logger.error("Error deleteing designate recored %s,"
                                "response status %s text %s" %
                                (rec_name, resp.status_code, resp.text)))
            return
    ...
    def __call__(self, env, start_response):
        def _d_start_response(status, headers, exc_info=None):
            try:
                if ('POST' in env.get('REQUEST_METHOD', '') and
                    ...
                    self._logger.debug('status %s for floatingips post',
                                       status)
                    self._status = status.split(' ')[0]
                elif ('DELETE' in env.get('REQUEST_METHOD', '') and
                        '/floatingips' in env.get('PATH_INFO', '')):
                    self._logger.debug('status %s for floatingips delete',
                                       status)
                    self._status = status.split(' ')[0]
            finally:
        ...
        if ('DELETE' in env.get('REQUEST_METHOD', '') and
            ...
            try:
                ...
                resp = requests.get(fip_url,
                                    headers={'X-Auth-Token':
                                             env['HTTP_X_AUTH_TOKEN']})
                if resp.status_code != 200:
                    raise Exception('status %s for floating'
                                    'ip read' % (resp.status_code))

                fip_dict = json.loads(resp.text)['floatingip']
                self._logger.debug('Read in floating ip info of %s for'
                                   'delete of %s', fip_dict, fip_id)
            except Exception as e:
                self._logger.exception('Exception %s in reading'
                                       'floating ip info', e)
        ...
        try:
            if ('POST' in env.get('REQUEST_METHOD', '') and
                ...
                if self._status not in ('200', '201'):
                    self._logger.error('Status %s in floating'
                                       ' ip create', self._status)
                    return
                ...
                if not dc_dom_id:
                    ...
                    dc_dom_id = self._create_designate_domain(dc_dom_name,
                                                              admin_token,
                                                              email, ttl)
                ####
                ...
                if not fip_dom_id:
                    self._logger.debug('Designate domain %s not found, '
                                       'creating...', fip_dom_name)
                    ## Creating NS record in dc_domain(fip_tld)
                    #dsg_ns_srvs =
                    #self._find_designate_server(self._admin_token, env) #FIXME
                    dsg_ns_srvs = (self._find_designate_server(
                        env['HTTP_X_AUTH_TOKEN'], env))
                    if dsg_ns_srvs:
                        for srv in dsg_ns_srvs:
                            self._create_designate_record(dc_dom_id,
                                                          fip_dom_name,
                                                          self._admin_token,
                                                          'NS', srv)
                    ###
                    ...
                    ttl = conf.get('ttl', 3600)
                    fip_dom_id = (self._create_designate_domain(fip_dom_name,
                                  env['HTTP_X_AUTH_TOKEN'], email, ttl))
                ...
                self._create_designate_record(fip_dom_id, fip_rec_name,
                                              env['HTTP_X_AUTH_TOKEN'], 'A',
                                              fip_addr)
### Need to complete DELETE
            elif ('DELETE' in env.get('REQUEST_METHOD', '') and
                  '/floatingips' in env.get('PATH_INFO', '')):
                if self._status not in ('200', '204'):
                    self._logger.error('Status %s in floating ip'
                                       'delete for %s', self._status,
                                       env.get('PATH_INFO', ''))
                    return
                fip_dom_name = self._get_fip_domain_name(env)
                dom_id = self._find_designate_domain(fip_dom_name, env)
                if not dom_id:
                    self._logger.error('Designate domain %s not found',
                                       fip_dom_name)
                    return
                ...
                if not rec_id:
                    self._logger.error('Designate record %s not found',
                                       fip_rec_name)

                self._delete_designate_record(dom_id, fip_rec_name, rec_id,
                                              env['HTTP_X_AUTH_TOKEN'])
        except Exception as e:
            self._logger.exception('Exception %s in %s floating ip'
                                   'to designate', e,
